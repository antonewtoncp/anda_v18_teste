...
class ClearanceWizard(models.TransientModel):
    ...
    def make_close_year(self):
        year = self.year
        company_id = self.env.user.company_id.id
        period = self.env['account.fiscal.period'].search(
            [('company_id', '=', company_id), ('year.name', '=', year.name), ('period', '=', '15')])
        ...
        journal = self.env['account.journal'].search([('name', 'ilike', 'divers'), ('company_id', '=', company_id)])
        ...
        move = self.env['account.move'].create({
            'ref': f"Fecho do Ano Fiscal {datetime.strftime(year.date_to, '%Y')}",
            'period': period.id,
        ...
        move.make_close_year(self.date_end, self.date_start)
