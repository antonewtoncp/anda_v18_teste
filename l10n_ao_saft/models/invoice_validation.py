...
class AccountMove(models.Model):
        ...
        for record in self:
            ...
            if record.move_type == 'out_invoice':
                ...
                if record.invoice_date and record.invoice_date > fields.Date.today():
                    raise ValidationError("A data da fatura não pode ser superior à data atual.")
    ...
    #  Fatura em estado publicado não pode ser trasformado em rascunho

    def button_draft(self):
        pass
        # for record in self:
        #     if record.state == 'posted':
        #         raise ValidationError("Não é permitido colocar uma fatura já publicada em rascunho.")
    ...
    def _check_withholding_threshold_warning(self):
        print("...... entrou .....")
        for record in self:
            withholding_tax = record.invoice_line_ids.mapped('tax_ids').filtered(lambda t: t.tax_type_with)
            if withholding_tax and record.price_subtotal < min(withholding_tax.mapped('threshold_wht'), 20000):
                warning_mess = {
                    'title': ('Atenção!'),
                    'message': ('O valor da fatura aplicado à retenção não pode ser inferior a 20.000 Kz.')
                }
                return {'warning': warning_mess}

    @api.constrains('invoice_line_ids', 'state')
    def _check_invoice_line_validations(self):
        for invoice in self.filtered(lambda inv: inv.state != 'draft' and inv.move_type == 'out_invoice'):
            for line in invoice.invoice_line_ids:
                ...
                if not line.product_uom_id:
                    raise ValidationError(('Incompleto! Todas as linhas da fatura devem ter uma unidade de medida.'))
                ...
                if line.quantity <= 0:
                    raise ValidationError(('Incompleto! Todas as linhas da fatura devem ter uma quantidade.'))

    @api.constrains('invoice_date_due')
    def _check_invoice_date_maturity(self):
        for record in self:
            if record.invoice_date_due and record.move_type == 'out_invoice' and record.invoice_date and record.invoice_date_due < record.invoice_date:
                raise ValidationError("A data de Vencimento nao pode ser menor que a data da factura.")
...
class AccountMoveLine(models.Model):
    ...
    def _onchange_invoice_line_tax_ids(self):
        for invoice_line in self:
            if invoice_line.move_id.move_type == 'out_invoice' and len(invoice_line.tax_ids) >= 2:
                warning_mess = {
                    'title': 'Atenção!',
    ...
    def _onchange_tax_withholding_threshold(self):
        for line in self:
            if line.move_id.move_type == 'out_invoice':
                ...
                if any(tax.name == 'RF II 6,5' for tax in line.tax_ids) and line.price_subtotal < 20000:
                    warning_mess = {
                        'title': 'Atenção!',
                        'message': ('Para retenção o valor mínimo aplicado deve ser superior ou igual a 20.000 Kz.\n'
    ...
    def _check_valuePositivoQ(self):
        for invoice_line in self:
            if invoice_line.move_id.move_type == 'out_invoice' and invoice_line.quantity < 0:
